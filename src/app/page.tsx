            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              <div className="grid md:grid-cols-2 gap-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Keyword List 1 (one per line)</FormLabel>
                      <FormControl>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4">
                ...
                <Button type="submit" disabled={isSearching || totalChecks > MAX_DOMAINS || totalChecks === 0} className="w-full sm:w-auto">
        ...
        <Card className="border-accent/50">
          <CardHeader>
            <CardTitle className="text-accent flex items-center">
              <Check className="mr-2"/> Available Domains ({availableDomains.length})
            </CardTitle>
            <CardDescription>Domains ready for registration.</CardDescription>
          </CardHeader>
          <CardContent>
            {availableDomains.length > 0 ? (
              <ul className="space-y-2">
                {availableDomains.map((d) => (
                  <li key={d.domain} className="flex justify-between items-center p-2 rounded-md hover:bg-white/5">
                    <span className="font-mono text-accent">{d.domain}</span>
                    <div className="flex items-center gap-2">
                        <span className="text-sm text-foreground">${d.price?.toFixed(2)}</span>
                        <Button variant="ghost" size="icon" onClick={() => copyToClipboard(d.domain)}>
                           {copiedDomain === d.domain ? <Check className="h-4 w-4 text-accent" /> : <Copy className="h-4 w-4" />}
                        </Button>
                    </div>
                  </li>
                ))}
                ...
                <p className="text-sm text-muted-foreground text-center py-8">
                    {isSearching ? "Searching for available domains..." : "No available domains found from this search."}
                </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center">
                    Unavailable Domains ({unavailableDomains.length})
                </CardTitle>
                 <CardDescription>These domains are already taken.</CardDescription>
            </CardHeader>
            <CardContent>
            {unavailableDomains.length > 0 ? (
                 <ScrollArea className="h-96">
                    <ul className="space-y-2 pr-4">
                        ...
                        <li key={d.domain} className="flex justify-between items-center p-2 rounded-md">
...
// Dummy component to satisfy import if not present
const ScrollArea: React.FC<{children: React.ReactNode, className?: string}> = ({children, className}) => <div className={className}>{children}</div>;
